...
export class EventService {
  ...
  //   localStorage.setItem("loginEmail", data.empNo);
  //   console.log(data.empNo);
  //   console.log(data.userName);
  //   return this.http
  //     .post<any>(Constant.API_ENDPOINT + "authenticate", data)
  //     .pipe(catchError(this.errorHandler));
  // }
  getEventById(id): Observable<Event[]> {
    return this.http
      .get<Event[]>(Constant.API_ENDPOINT + "/rest/events/" + id)
      .pipe(catchError(this.errorHandler));
  }

  createEvent(EventData): Observable<Event[]> {
    return this.http
      .post<any>(Constant.API_ENDPOINT + "/rest/events", EventData)
      .pipe(catchError(this.errorHandler));
  }

  updateEvent(EventData): Observable<Event[]> {
    return this.http
      .put<any>(Constant.API_ENDPOINT + "/rest/events", EventData)
      .pipe(catchError(this.errorHandler));
  }
